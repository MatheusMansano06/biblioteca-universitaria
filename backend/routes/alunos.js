...
// LISTAR TODOS (usado antes nos testes)
router.get('/', (req, res) => {
...
router.post('/', (req, res) => {
  ...
  if (!validateRA(ra))
    return res.status(400).json({ error: 'RA deve conter exatamente 8 números.' });
  if (!validateEmail(email))
    return res.status(400).json({ error: 'Email deve ser institucional (@puccampinas.edu.br).' });
  if (!validateTelefone(telefone))
    return res.status(400).json({ error: 'Telefone deve estar no formato (19) 9####-####.' });
  ...
  db.query(checkSQL, [ra], (err, r) => {
    ...
    const insertSQL = 'INSERT INTO alunos (nome, ra, curso, email, telefone) VALUES (?, ?, ?, ?, ?)';
